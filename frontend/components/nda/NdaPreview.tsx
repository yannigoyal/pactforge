    ...
    <div className="flex flex-col gap-3 text-sm leading-relaxed text-slate-800">
      {blocks.map((block, index) => {
        switch (block.kind) {
          case "heading":
            return (
              <h3 key={index} className="text-center text-base font-semibold text-slate-900">
          ...
          case "footer":
            return (
              <div key={index} className="mt-2 border-t border-slate-200 pt-3 text-xs text-slate-500">
...
export function NdaPreview({ values, bodyBlocks }: NdaPreviewProps) {
  return (
    <div className="flex flex-col gap-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">Cover Page for Bonterms Mutual NDA</h2>
      ...
      {values.additionalTerms ? (
        <div className="text-sm">
          <p className="font-medium text-slate-600">Additional Terms</p>
          <p className="whitespace-pre-wrap text-slate-900">{values.additionalTerms}</p>
        </div>
      ) : null}
